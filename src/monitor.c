...
static int monitor_particle_velocity(
  ...
  //   of the object
  // normalize fft here
  const double particle_velocity[NDIMS] = {
    - surface_concentration[     1] / ny,
    - surface_concentration[ny - 1] / ny,
  };
  const char file_name[] = "output/particle_velocity.dat";
  errno = 0;
  ...
  fprintf(fp, "% .7e % .7e % .7e\n", time, particle_velocity[0], particle_velocity[1]);
  fclose(fp);
  return 0;
}
...
int monitor(
  ...
  monitor_particle_velocity(domain, time, surface_concentration, fopen_mode);
